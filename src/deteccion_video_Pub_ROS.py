...
# EL PROGRAMA DETECTA SI HAY UNA VICTIMA CON ETIQUETA CUERPO VICTIMA, PARA OBTENER SUS COORDENADAS Y PUBLICARLAS POR UN NODO
from models import *
...
if __name__ == "__main__":
    ...
    parser.add_argument("--img_size", type=int, default=REZ, help="size of each image dimension")
    ...
    while cap:
        ...
        imgTensor = resize(imgTensor, REZ)
        ...
        for detection in detections:
            if detection is not None:
                ...
                for x1, y1, x2, y2, conf, cls_conf, cls_pred in detection:
                    ...
                    # ADD
                    if classes[int(cls_pred)]=="victima_cuerpo":
                        print("*********************")
                        p = Pose()
                        p.position.x = (x2 + x1)/2
                        p.position.y = (y2 + y1)/2
                        p.position.z = 1.0
